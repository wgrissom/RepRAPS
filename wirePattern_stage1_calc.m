% function: wirePattern_stage1_calc - plot point values of optimized power
...
remaining_tl_calc = remaining_tl-(19*(pi*r/2)); %minimum of 19 segments in base pattern (see Minimum # of Segments), approximation bc some will be stretched/shrunk
...
[startX,startY,l] = draw('c',startX,startY + 2*r,r,w,sf); %cannot vary --> sf = 0
remaining_tl = remaining_tl-l;
[startX,startY,l] = draw('single',startX,startY+2*r,r,w,0,3*pi/2); %cannot vary --> sf = 0
remaining_tl = remaining_tl-l;
...
countB = 0;
countA = 0;
while remaining_tl_calc > 2*4*(pi*r/2)- 6   %~6mm is a reasonable adjustment in trace length through sf, 2*(length that will be needed in 2nd while loop
...
while remaining_tl_calc > (4*(pi*r/2)) - 6 %~6mm is a reasonable adjustment in trace length through sf
    [startXpA,startYpA,l] = draw('wave_upR',startXpA,startYpA,r,w,0); %cannot vary --> sf = 0
    remaining_tl = remaining_tl-l;
    remaining_tl_calc = remaining_tl_calc - 2*l;
    countA = countA + 1;
end
...
[startX,startY,l] = draw('top_capR_V',startXpA,startYpA,r,w,sf);
remaining_tl = remaining_tl-l;
while countA > 0
    [startX,startY,l] = draw('wave_downL',startX,startY,r,w,0); %cannot vary --> sf = 0
    remaining_tl = remaining_tl-l;
...
[startX,startY,l] = draw('c',startX,startY - 2*r,r,w,sf); %cannot vary --> sf = 0
remaining_tl = remaining_tl-l;
[startX,startY,l] = draw('single',startX,startY-2*r,r,w,0,0); %cannot vary --> sf = 0
remaining_tl = remaining_tl-l;
%hold space to add variable trace length later
startXpA = startX;
startYpA = startY;
startX = startX + 2*r;
[startX,startY,l] = draw('top_capL_V',startX,startY,r,w,sf); %cannot stretch - too close to Z2 (sf <= 0 here)
remaining_tl = remaining_tl-l;
%hold space to add variable trace length later
startXpB = startX;
startYpB = startY;
...
[startX,startY,l] = draw('top_capL_V',startX,startY,r,w,sf); %cannot stretch - too close to Z2 (sf <= 0 here)
remaining_tl = remaining_tl-l;
%variation in radius of 'u' allows Z1 to end at POR
if (POR-(startX+r))/2 < 1.25*r % radius > 1.25*calculated radius starts to look weird
...
while remaining_tl_calc > (4*(pi*r/2)) - 6 %~6mm is a reasonable adjustment in trace length through sf
    [startXpA,startYpA,l] = draw('wave_downR',startXpA,startYpA,r,w,0); %cannot vary --> sf = 0
    remaining_tl = remaining_tl-l;
    remaining_tl_calc = remaining_tl_calc - 2*l;
    countA = countA + 1;
end
...
[startX,startY,l] = draw('bot_capR_V',startXpA,startYpA,r,w,sf);
remaining_tl = remaining_tl-l;
while countA > 0
    [startX,startY,l] = draw('wave_upL',startX,startY,r,w,0); %cannot vary --> sf = 0
    remaining_tl = remaining_tl-l;
...
%calculation of initial stretch/shink factor
if remaining_tl_calc < 0 %if the approximation already is too long, sf < 0
    sf = remaining_tl_calc/3; %overshoot (minimum_length - desired_length) / 3 (There are 3 locations in the remaining pattern that can be stretch/shrunk)
else
   sf = 0; %ignore until postive sf is needed to increase trace length
end
if sf < -4  %~6mm is a reasonable adjustment in trace length through sf
    startXpA = startX;
    startYpA = startY;
    startX = startX + 2*r;
    startY = startY + 2*r;
    [startX,startY,l] = draw('wave_downR',startX,startY,r,w,0); %cannot stretch/shrink --> sf = 0 
    remaining_tl = remaining_tl-l;
    startXpB = startX;
    startYpB = startY;
    startX = startX + 2*r;
    [startX,startY,l] = draw('c',startX,startY+ 2*r,r,w,0);  %cannot stretch/shrink --> sf = 0 
    remaining_tl = remaining_tl-l;
    [startX,startY,l] = draw('single',startX,startY+2*r,r,w,0,3*pi/2); %cannot stretch/shrink --> sf = 0 
    remaining_tl = remaining_tl-l;
    ...
    remaining_tl_calc = remaining_tl-(9*(pi*r/2)); %minimum of 9 segments remaining in base pattern (see Minimum # of Segments), approximation bc some will be stretched/shrunk
    sf = remaining_tl_calc/3; %overshoot (minimum_length - desired_length) / 3 (There are 3 locations in the remaining pattern that can be stretch/shrunk)
    [startX,startY,l] = draw('bot_capR_V',startXpA,startYpA,r,w,sf); %cannot stretch - too close to Z4 (sf <= 0 here)
    remaining_tl = remaining_tl-l;
    [startX,startY,l] = draw('top_capL_V',startX,startY,r,w,sf); %cannot stretch - too close to Z4 (sf <= 0 here)
    remaining_tl = remaining_tl-l;
    [startX,startY,l] = draw('bot_capR_V',startXpB,startYpB,r,w,sf); %cannot stretch - too close to Z4 (sf <= 0 here)
    remaining_tl = remaining_tl-l;
else
    [startX,startY,l] = draw('bot_capR_V',startX,startY,r,w,sf); %cannot stretch - too close to Z4 (sf <= 0 here)
    remaining_tl = remaining_tl-l;
    [startX,startY,l] = draw('c',startX,startY+2*r,r,w,0); %cannot stretch/shrink --> sf = 0 
    remaining_tl = remaining_tl-l;
    [startX,startY,l] = draw('single',startX,startY+2*r,r,w,0,3*pi/2); %cannot stretch/shrink --> sf = 0 
    ...
    [startX,startY,l] = draw('c',startX,startY-2*r,r,w,0); %cannot stretch/shrink --> sf = 0 
    remaining_tl = remaining_tl-l;
    [startX,startY,l] = draw('single',startX,startY-2*r,r,w,0,0); %cannot stretch/shrink --> sf = 0 
    remaining_tl = remaining_tl-l;
    %hold space to add variable trace length later
    startXpB = startX;
    startYpB = startY;
    ...
    count = 0;
    while remaining_tl_calc > 4*(pi*r/2)- 2*6  %~6mm is a reasonable adjustment in trace length through sf * 2 locations for sf
        [startXpA,startYpA,l] = draw('wave_upR',startXpA,startYpA,r,w,0); %cannot vary --> sf = 0
        remaining_tl = remaining_tl-l;
        remaining_tl_calc = remaining_tl_calc - 2*l;
        count = count + 1;
    ...
    remaining_tl = remaining_tl-l;
    while count > 0
        [startX,startY,l] = draw('wave_downL',startX,startY,r,w,0); %cannot stretch/shrink --> sf = 0
        remaining_tl = remaining_tl-l;
        count = count - 1;
    end
    [startX,startY,l] = draw('bot_capR_V',startXpB,startYpB,r,w,sf);
...
% function: Z4 - plot point valu es of optimized WPS's trace 4
...
% out1: startX - x coordinate on wire pattern plot after Z4 (mm)
% out2: startY - y coordinate on wire pattern plot after Z4 (mm)
% out3: remaining_tl - actual - desired trace length of Z4 (mm)
...
remaining_tl = tl;
%remaining_tl_calc - a running approximation of the trace length not yet
remaining_tl_calc = remaining_tl-(19*(pi*r/2)); %minimum of 15 segments in base pattern (see Minimum # of Segments), approximation bc some will be stretched/shrunk
%calculation of initial stretch/shink factor
if remaining_tl_calc < 0 %if the approximation already is too long, sf < 0
    sf = remaining_tl_calc/3; %overshoot (minimum_length - desired_length) / 3 (There are 3 locations in the remaining pattern that can be stretch/shrunk)
else
...
if sf < -4  %~6mm is a reasonable adjustment in trace length through sf
    ...
    startX = startX + 2*r;
    startY = startY - 2*r;
    [startX,startY,l] = draw('wave_upR',startX,startY,r,w,0); %cannot stretch/shrink --> sf = 0 
    remaining_tl = remaining_tl-l;
    startXpB = startX;
    startYpB = startY;
    startX = startX + 2*r;
    ...
    remaining_tl = remaining_tl-l;
    %variation in radius of 'single' allows Z3 to end at LOB-2*LIOP (beginning of output port)
    [startXe,startYe,l] = draw('single',startX+2*r+abs(DBP/2+startY)-r,startY,abs(DBP/2+startY),w,0,pi); %cannot stretch/shrink --> sf = 0
    line([startXe,LOB-2*LIOP],[startYe + -DBP/2-startY + w,startYe + -DBP/2-startY + w],'Color','red');
    line([startXe,LOB-2*LIOP],[startYe + -DBP/2-startY - w,startYe + -DBP/2-startY - w],'Color','red');
    remaining_tl = remaining_tl-l - (LOB-2*LIOP - startXe);
    remaining_tl_calc = remaining_tl-(9*(pi*r/2)); %minimum of 9 segments remaining in base pattern (see Minimum # of Segments), approximation bc some will be stretched/shrunk
    sf = remaining_tl_calc/3; %overshoot (minimum_length - desired_length) / 3 (There are 3 locations in the remaining pattern that can be stretch/shrunk)
    [startX,startY,l] = draw('top_capR_V',startXpA,startYpA,r,w,sf); %cannot stretch - too close to Z4 (sf <= 0 here)
    remaining_tl = remaining_tl-l;
    [startX,startY,l] = draw('bot_capL_V',startX,startY,r,w,sf); %cannot stretch - too close to Z4 (sf <= 0 here)
    remaining_tl = remaining_tl-l;
    [startX,startY,l] = draw('top_capR_V',startXpB,startYpB,r,w,sf); %cannot stretch - too close to Z4 (sf <= 0 here)
    remaining_tl = remaining_tl-l;
else
    [startX,startY,l] = draw('top_capR_V',startX,startY,r,w,sf); %cannot stretch - too close to Z3 (sf <= 0 here)
    remaining_tl = remaining_tl-l;
    [startX,startY,l] = draw('c',startX,startY-2*r,r,w,0); %cannot stretch/shrink --> sf = 0 
    remaining_tl = remaining_tl-l;
    [startX,startY,l] = draw('single',startX,startY-2*r,r,w,0,0); %cannot stretch/shrink --> sf = 0 
